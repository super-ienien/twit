...
var API = {STREAMING: 'streaming', REST: 'rest', CURATOR: 'curator'};
...
// config values required for curator auth
var required_for_curator_auth = [
  'curator_auth_token'
];
...
var Twitter = function (config) {
  ...
  this.config = config;
  this._twitter_time_minus_local_time_ms = StreamingAPIConnection._twitter_time_minus_local_time_ms= 0;
}
...
Twitter.prototype.request = function (method, path, params, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    self._buildReqOpts(method, path, params, API.REST, function (err, reqOpts) {
      if (err) {
...
Twitter.prototype._updateClockOffsetFromResponse = function (resp) {
  ...
  if (resp && resp.headers && resp.headers.date &&
    ...
    self._twitter_time_minus_local_time_ms = StreamingAPIConnection._twitter_time_minus_local_time_ms = twitterTimeMs - Date.now();
  }
}

Twitter.prototype.curator = function (path, params, callback) {
  var self = this
  var method = 'GET';
  ...
  self._buildReqOpts(method, path, params, API.CURATOR, function (err, reqOpts) {
    if (err) {
      callback(err, null, null)
      return
    }
    ...
    process.nextTick(function () {
      // ensure all HTTP i/o occurs after the user has a chance to bind their event handlers
      self._doCuratorApiRequest(reqOpts, twitOptions, method, callback)
    })
  })
 ...
 * @param  {String}  API Flag indicating which API to connect to (differents endpoints)
 * @returns {Undefined}
 *
 ...
 */
Twitter.prototype._buildReqOpts = function (method, path, params, api, callback) {
  var self = this
  if (!params) {
    params = {}
  }
  ...
  switch (api) {
    ...
    case API.REST :
      ...
      break;
    case API.CURATOR :
      reqOpts.url = endpoints.CURATOR_ROOT + path + '.json'
      reqOpts.headers['Content-type'] = 'application/json'
      break;
  }
  ...
  switch (api) {
    ...
    case API.REST:
      ...
      break;
    case API.CURATOR:
      if (!self.config.curator_auth_token) {
        callback(new Error ('Config must provide a curator auth token', null));
      } else {
        reqOpts.qs = {auth : self.config.curator_auth_token};
        callback(null, reqOpts);
      }
    break;
  }
...
/**
 * Make HTTP request to Twitter CURATOR API.
 * @param  {Object}   reqOpts     options object passed to `curator()`
 * @param  {Object}   twitOptions
 * @param  {String}   method      "GET" or "POST"
 * @param  {Function} callback    user's callback
 * @return {Undefined}
 */
Twitter.prototype._doCuratorApiRequest = function (reqOpts, twitOptions, method, callback) {
  ...
  var onRequestComplete = function () {
    ...
    if (body.hasOwnProperty('status') && body.status.code != 200)
	{
      // we got a Twitter API-level error response
      // place the errors in the HTTP response body into the Error object and pass control to caller
      var err = helpers.makeTwitError(body.status.message);
      err.statusCode = body.status.code;
      callback(err, body, response);
      return
    }
  ...
  req.on('error', function (err) {
    ...
    if (twitOptions.retry &&
      ...
      // retry the request since retries were specified and we got a status code we should retry on
      self._doCuratorApiRequest(reqOpts, twitOptions, callback) ;
      return;
    } else {
      // pass the transport-level error to the caller
      err.statusCode = null
...
Twitter.prototype.stream = function (path, params) {
  ...
  self._buildReqOpts('POST', path, params, API.STREAMING, function (err, reqOpts) {
    if (err) {
...
Twitter.prototype._validateConfigOrThrow = function (config) {
  ...
  } else if (config.curator_only) {
    var auth_type = 'curator auth';
    var required_keys = required_for_curator_auth;
  } else {
