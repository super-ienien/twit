...
StreamingAPIConnection.prototype._startPersistentConnection = function () {
  ...
  self.request.on('response', function (response) {
    ...
    if (STATUS_CODES_TO_ABORT_ON.indexOf(self.response.statusCode) !== -1) {
      ...
      gunzip.on('error', function (err) {
        ...
        var errMsg = 'Gzip error on twitter disconnect event : ' + err.message;
        var twitErr = helpers.makeTwitError(errMsg);
        twitErr.statusCode = self.response.statusCode;
        helpers.attachBodyInfoToError(twitErr, body+' ------------------------------------------------------------------------- '+compressedBody);
      ...
      gunzip.on('error', function (err) {
        ...
        var twitErr = helpers.makeTwitError(errMsg);
        twitErr.statusCode = self.response.statusCode;
        helpers.attachBodyInfoToError(twitErr, {gunzip: gunzip, response: self.response});
        self.emit('error', twitErr);
      })
  ...
  self.request.on('error', function (err) { console.error (err); self._scheduleReconnect.bind(self) });
  return self;
}
